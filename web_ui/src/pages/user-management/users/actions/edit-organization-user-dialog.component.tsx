...
export const EditOrganizationUserDialog = ({
  ...
  const currentOrgRole: USER_ROLE | undefined = useMemo(
    () => user.roles.find(r => r.resourceType === RESOURCE_TYPE.ORGANIZATION && r.resourceId === organizationId)?.role,
  ...
  const orgAdmins = useMemo(
    () => users.filter(u => u.roles.some(r => r.resourceType === RESOURCE_TYPE.ORGANIZATION && r.resourceId === organizationId && r.role === USER_ROLE.ORGANIZATION_ADMIN)),
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (roleChanged && isActiveOrgAdmin && selectedOrgRole && !(isLastRemainingOrgAdmin && selectedOrgRole !== USER_ROLE.ORGANIZATION_ADMIN)) {
      if (FEATURE_FLAG_MANAGE_USERS_ROLES) {
        ...
        promises.push(updateUserRoles.mutateAsync({ newRoles: [del, create], userId: user.id, organizationId }));
      }
    }
